...
import { AnimateCartoonConfig, Animator } from '../animate';
import { setCtxStyleProp } from '../renderer/canvas';
import {
  createRectPath2D,
  setCirclePath2D,
} from '../renderer/canvas/setPath2D';
...
export abstract class AbstractUi<T = {}> extends AbsEvent {
  ...
  attrDirty(data: Partial<T>) {
    ...
    clipRect(ctx, createRectPath2D(sb), () => {
      ...
      const correctSortedShapes = this.stage.children.filter((item) =>
        neededUpdatedShapes.includes(item),
      );
  ...
  animateCartoon(targetProp: Partial<T>, cfg: AnimateCartoonConfig = {}) {
    ...
    const startProp = Object.keys(targetProp).reduce(
      (acc, k) => Object.assign(acc, { [k]: this.data[k] }),
      {},
    );
...
export function isRectShapeCollision(
  rect_1: BoundingRect,
  rect_2: BoundingRect,
) {
